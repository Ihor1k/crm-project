...
export function ExperimentsPage({ currentRoute = "/experiments" } = {}) {
  ...
  const rows = [
    row("Homepage headline test", "Spring Promotion", "Running", "50% | 50%"),
    row("CTA color test", "Retention Flow", "Completed", "70% | 30%"),
    row("Email subject line test", "Re-engagement Campaign", "Draft", "12% | 88%"),
    row("Signup form length test", "User Acquisition", "Running", "50% | 50%"),
    row("Hero image", "Summer Promotion", "Draft", "50% | 50%"),
    row("Pricing layout test", "Spring Promotion", "Draft", "70% | 30%"),
    row("Checkout copy test", "Retention Flow", "Completed", "70% | 30%"),
    row("Nav CTA position test", "Summer Promotion", "Running", "50% | 50%"),
    row("Landing page testimonial test", "Spring Promotion", "Running", "50% | 50%"),
    row("Email CTA placement test", "Re-engagement Campaign", "Completed", "70% | 30%"),
  ];
    ...
    <main class="dashboard-layout">
      ...
      <section class="dashboard-main">
        <header class="dashboard-header">
          <div>
            <h1>Experiments</h1>
            <p>A/B tests overview</p>
          </div>
        </header>

        <section class="campaign-toolbar" aria-label="Experiments actions">
          ...
          <div class="campaign-toolbar__actions">
            ...
            <div class="crm-filter" data-filter-popover aria-hidden="true">
              ...
              <label class="crm-filter__field">
                <span class="crm-filter__label">Test name starts with</span>
        ...
        <section class="panel campaign-panel" aria-label="Experiments table">
...
function row(testName, campaignName, status, trafficSplit) {
  return { testName, campaignName, status, trafficSplit };
}
...
function renderRow(item) {
  return `
    <tr data-test-name="${escapeHtmlAttr(item.testName)}">
      ...
      <td>${escapeHtml(item.trafficSplit)}</td>
    </tr>
  `;
}
